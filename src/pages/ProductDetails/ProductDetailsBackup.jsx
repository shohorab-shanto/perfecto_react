...
const ProductDetails = () => {
    ...
    const handleSelectedShadeData = (id) => {
        ...
        const selectedShade = singleProduct?.data?.product?.product_shades.find((shade) => shade.shade_id == id);
        if (selectedShade) {
    ...
    const handleSelectedSizeData = (id) => {
        const selectedSize = singleProduct?.data?.product?.product_sizes.find((size) => size.size_id == id);
        if (selectedSize) {
    ...
    useEffect(() => {
        if (!isLoading && singleProduct?.data?.product?.product_shades?.length > 0) {
            const availableShadesOnStock = singleProduct?.data?.product?.product_shades?.filter((shade) => shade?.stock !== null && shade?.stock != 0);
            ...
            if (initialSelectedShadeId) {
                ...
                handleSelectedShadeData(singleProduct?.data?.product?.product_shades?.[0]?.shade_id);
            }
        } else if (!isLoading && singleProduct?.data?.product?.product_sizes?.length > 0) {
            const availableSizesOnStock = singleProduct.data.product?.product_sizes?.filter((size) => size?.stock !== null && size?.stock != 0);
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e) => {
            ...
            if (e.key === "ArrowRight") {
                const selectedShadeIndex = singleProduct?.data?.product?.product_shades?.findIndex(
                    (shade) => parseInt(shade.shade_id) === parseInt(selectedProduct?.shade?.id)
                );
                if (selectedShadeIndex < singleProduct?.data?.product?.product_shades?.length - 1) {
                    const nextIndex = selectedShadeIndex + 1;
                    setSelectedShadeIndex(nextIndex);
                    handleSelectedShadeData(singleProduct?.data?.product?.product_shades[nextIndex]?.shade?.id); // product?.shade?.id
                }
                ...
                if (selectedShadeIndex > 0) {
                    ...
                    handleSelectedShadeData(singleProduct?.data?.product?.product_shades[prevIndex]?.shade?.id);
                }
            }
        };
    ...
    useEffect(() => {
        if (Array.isArray(singleProduct?.data?.product?.product_shades) && singleProduct?.data?.product?.product_shades?.length > 0) {
            const result = singleProduct?.data?.product?.product_shades?.map((item) => ({
                label: item?.shade?.name,
                key: `${item?.shade?.id}`,
            }));

            setDropDownItemsForShade(result);
        } else if (Array.isArray(singleProduct?.data?.product?.product_sizes) && singleProduct?.data?.product?.product_sizes?.length > 0) {
            const result = singleProduct?.data?.product?.product_sizes?.map((item) => ({
                    ...
                    <div className=" container mx-auto">
                        ...
                        <div className="rounded mx-[10px]" style={{ boxShadow: "0px 0px 6px 0px #E4EDF0" }}>
                            <div className="md:flex justify-between gap-x-4 xl:gap-x-0 my-3">
                                        ...
                                        <div className="px-1 md:ps-5 md:pt-5 md:pe-5 md:pb-3">
                                            <div>
                                                ...
                                                <div className="flex flex-wrap items-center gap-x-2 mb-2 md:mb-3">
                                                    {singleProduct?.data?.product?.brand_id && singleProduct?.data?.product?.brand?.name && (
                                                        <>
                                                            <p className="whitespace-nowrap text-[#000000CC] font-inter text-xs md:text-sm font-normal leading-normal">
                                                                ...
                                                                <Link to={`/product-filter?brand=${singleProduct?.data?.product?.brand_id}`}>
                                                                    <span
                                                                        className="text-black font-medium hover:cursor-pointer"> {singleProduct?.data?.product?.brand?.name}</span>
                                                                </Link>
                                                            </p>
                                                    ...
                                                    {singleProduct?.data?.product?.country !== null && (
                                                        <>
                                                            <div className="border-[0.1px] h-4"></div>

                                                            <p className="whitespace-nowrap text-[#000000CC] font-inter text-xs md:text-sm font font-normal leading-normal">
                                                                Country Of Origin: <span className="text-black font-medium">{singleProduct?.data?.product?.country?.name}</span>
                                                            </p>
                                                        </>
                                                    )}
                                                </div>
                                                <div className="flex flex-wrap items-center gap-x-2 mb-2 md:mb-3">
                                                    ...
                                                    <div className="text-black font-inter text-xs md:text-sm font-normal leading-normal flex items-center">
                                                        <span className="text-black font-medium text-lg">
                                                            {singleProduct?.data?.product?.reviews_avg_star == null
                                                                ? '0.00'
                                                                : parseFloat(singleProduct?.data?.product?.reviews_avg_star).toFixed(2)}
                                                            /5
                                                        </span>
                                                ...
                                                <div className="bg-[#EDEFF0] py-3 ps-2 md:ps-4 mb-3 md:mb-4">
                                                    <ul className=" ps-4">
                                                        {isReviewMoreOffer ? (
                                                            <>
                                                                {selectedProduct?.offers?.map((offer, i) => (
                                                                    <span key={i}>
                                                                        <Link
                                                                            to={`/campaign/${offer?.product_details?.offer_id
                                                                                    ? offer?.product_details?.offer_id
                                                                                    : offer?.product_details?.offer_combo?.offer_id
                                                                                }`}
                                                                        >
                                                                            <li className="list-disc text-[#000000CC] font-Inter text-xs xs:text-sm font-normal leading-normal hover:cursor-pointer">
                                                                                {offer?.product_details?.offer?.title1 == null
                                                                                    ? offer?.product_details?.offer?.name
                                                                                    : offer?.product_details?.offer?.title1}
                                                                                {offer?.product_details?.offer_combo?.offer?.title1 == null
                                                                                    ? offer?.product_details?.offer_combo?.offer?.name
                                                                                    : offer?.product_details?.offer_combo?.offer?.title1}
                                                                            </li>
                                                                        </Link>
                                                                    </span>
                                                                ))}
                                                            ...
                                                            <>
                                                                {selectedProduct?.offers?.slice(0, 2)?.map((offer, i) => (
                                                                    <span key={i}>
                                                        ...
                                                        {selectedProduct?.offers?.length > 2 ? (
                                                            isReviewMoreOffer ? (
                                                                <li
                                                                    onClick={() => setIsReviewMoreOffer(false)}
                                                                    className=" text-[#0094CF] font-Inter text-xs xs:text-sm font-normal leading-normal hover:cursor-pointer"
                                                                >
                                                                    View Less
                                                                </li>
                                                            ) : (
                                                                <li
                                                                    onClick={() => setIsReviewMoreOffer(true)}
                                                                    className=" text-[#0094CF] font-Inter text-xs xs:text-sm font-normal leading-normal hover:cursor-pointer"
                                                                >
                                                                    View More Offers
                                                                </li>
                                ...
                                >
                                  <p className="text-[#000000CC] text-sm font-medium leading-normal">{selectedProduct?.shade?.name}</p>
                                ...
                                <div
                                                                    ...
                                                                    className={`absolute top-8 w-52 h-10 whitespace-nowrap bg-white rounded overflow-hidden overflow-y-auto slim-scroll z-20 ${
                                                                ...
                                                                >
                                                                    ...
                                                                    <p className="text-[#000000A6]   text-sm font-medium leading-normal hover:bg-primary-color p-2 hover:cursor-pointer">
                                                                        102 Nude Shade Color
                                                                    </p>
                                                                    <p className="text-[#000000A6]   text-sm font-medium leading-normal hover:bg-primary-color p-2 hover:cursor-pointer">
                                                                        301 Nude Shade Color
                                                                    </p>
                                                                    <p className="text-[#000000A6]   text-sm font-medium leading-normal hover:bg-primary-color p-2 hover:cursor-pointer">
                                                                        121 Nude Shade Color
                                                                    </p>
                                                                    <p className="text-[#000000A6]   text-sm font-medium leading-normal hover:bg-primary-color p-2 hover:cursor-pointer">
                                                                        332 Nude Shade Color
                                                                    </p>
                                                                </div>
                              </div> */}
                                                            <p className="text-[#000000CC] text-sm font-medium leading-normal border-b-2 border-[#5DC9F4]">
                                                                All Shades ({singleProduct?.data?.product?.all_shades_count})
                                                            </p>
                                                        </div>
                                                        <div className="flex items-start gap-2">
                                                            ...
                                                            <div className="border h-24 overflow-hidden overflow-y-auto slim-scroll p-2 w-full flex gap-2 flex-wrap">
                                                                {Array.isArray(singleProduct?.data?.product?.product_shades) && [...singleProduct?.data?.product?.product_shades]
                                                                    ?.map((product, index) => (
                                                                        <Fragment key={index}>
                                                                            {product?.stock == null ? (
                                                                                <Tooltip placement="top" title={"Stock Out"}>
                                                                                    <div
                                                                                        className={` rounded hover:cursor-pointer overflow-hidden relative ${product?.shade?.id == selectedShade?.shade_id
                                                ...
                                                absolute top-0 left-0 w-full h-full flex justify-center items-center`}
                                                                                ...
                                                                                >
                                                                                    <div
                                                                                        className={`${product?.shade?.id == selectedShade?.shade_id ? "block" : "hidden"
                                                                                            } absolute top-0 left-0 w-full h-full flex justify-center items-center`}
                                                                                    >
                                                    ...
                                                    <div className="mb-4">
                                                        <div className="flex items-center justify-between mb-1">
                                                            <div className="relative">
                                                                ...
                                                                >
                                                                    {singleProduct?.data?.product?.product_sizes?.map((item, index) => (
                                                                        <p
                                                                            key={index}
                                                                            className="text-[#000000A6]   text-sm font-medium leading-normal hover:bg-primary-color p-2 hover:cursor-pointer"
                                                                        >
                                                                            {item?.size?.name}
                                                                        </p>
                                                                    ))}
                                                        ...
                                                        <div className="flex items-start flex-wrap gap-2">
                                                            ...
                                                            {Array.isArray(singleProduct?.data?.product?.product_sizes) && [...singleProduct?.data?.product?.product_sizes]
                                                        ...
                                                        <div className="w-[104px] h-9 border rounded-md border-[#5DC9F4] flex items-center">
                                                            ...
                                                            >
                                                                ...
                                                                {updateCartIsLoading || isCartFetching ? (
                                                                    <TbMinus className=" text-[#5dc9f4a6] hover:cursor-not-allowed" />
                                                                ) : (
                                                                    <TbMinus onClick={() => handleAddedProductDecrement(cartItemIsExist)} className="hover:cursor-pointer" />
                                                                )}
                                                            </button>
                                                            <input
                                                                type="number"
                                                            ...
                                                            >
                                                                {/* <TbPlus /> */}
                                                                {updateCartIsLoading || isCartFetching ? (
                                                                    <TbPlus className=" text-[#5dc9f4a6] hover:cursor-not-allowed" />
                                                                ) : (
                                                                    <TbPlus onClick={() => handleAddedProductIncrement(cartItemIsExist)} className="hover:cursor-pointer" />
                                                                )}
                                                            </button>
                                                            {updateCartIsLoading || isCartFetching ? (
                                                ...
                                                <div className="flex items-center justify-between gap-1">
                                                    <div className="flex items-center gap-2 md:gap-5">
                                                        ...
                                                        <Button
                                                            ...
                                                            className={`rounded font-semibold  text-xs px-2 py-1 xs:py-2 xs:px-5 md:py-[8px] ${wishListItemIsExist == null || "bg-primary-color"
                                                                }`}
                                                            variant="bordered"
                                                        >
                        ...
                        <div className="flex items-start gap-7 ">
                            <div className="w-full">
                                ...
                                <div>
                                    ...
                                    {/* rating start */}
                                    <div className="rounded p-1 mb-10 md:mb-20 mx-[10px]" ref={reviewsSectionRef} id="reviews-section"
                                        ...
                                        <div className="py-2 md:px-5 md:pt-5">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-[#000000A6]  text-xs font-normal leading-normal">
                                                            {singleProduct?.data?.product?.reviews_count == null
                                                                ? 0
                                                                : parseFloat(singleProduct?.data?.product?.reviews_count).toFixed(0)}{" "}
                                                            verified ratings
                                                        </p>
                                                    </div>
                                                </div>
                                                    ...
                                                    <>
                                                        ...
                                                        {isCreateReview ? (
                                                            <CreateReview
                                                                ...
                                                                myReview={singleProduct?.data?.product?.my_review}
                                            ...
                                            <div className="border-t-[1px] border-b-[1px] overflow-hidden py-5">
                                                ...
                                                <Link
                                                    to={`/all-review/${id}?selected_image=true&&product_name=${singleProduct?.data?.product?.name}&&reviews_avg_star=${singleProduct?.data?.product?.reviews_avg_star}&&eligible=${singleProduct?.data?.product?.review_eligible}`}
                                                >
                                            ...
                                            <div className="">
                                                ...
                                                <Link
                                                    to={`/all-review/${id}?product_name=${singleProduct?.data?.product?.name}&&reviews_avg_star=${singleProduct?.data?.product?.reviews_avg_star}&&eligible=${singleProduct?.data?.product?.review_eligible}`}
                                                >
